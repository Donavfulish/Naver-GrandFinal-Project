...
export default function CapsuleOverview({ capsules }: CapsuleOverviewProps) {
  ...
  const avgDuration = Math.round(capsules.reduce((acc, c) => acc + c.duration, 0) / capsules.length)
  ...
  const commonTheme = capsules
    .reduce((acc: Record<string, number>, c) => {
      acc[c.vibe_config.theme] = (acc[c.vibe_config.theme] || 0) + 1
      return acc
    }, {})

  const mostCommonTheme = Object.entries(commonTheme).sort((a, b) => b[1] - a[1])[0]?.[0] || "rainy_night"
