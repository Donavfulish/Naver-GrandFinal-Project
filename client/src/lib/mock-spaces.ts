// mock-spaces.ts
export const mockSpaces = [
  {
    id: 1,
    name: "Focus Flow",
    tags: ["Focus", "Productive"],
    background: "/img/calming-ambient-environment.png",
    clockStyle: "minimal",
    clockFont: "Inter", // FONT FOR CLOCK
    img: "/img/calming-ambient-environment.png",
    currentTrack: "Lo-fi Beat 01",
    artist: "Lo-Fi Collective",
    albumArt: "/img/album1.png",
    playlist: ["Lo-fi Beat 01", "Study Flow", "Chillhop Focus"],
  },
  {
    id: 2,
    name: "Evening Calm",
    tags: ["Relax", "Calm"],
    background: "/img/minimalist-focus-workspace.png",
    clockStyle: "modern",
    clockFont: "Orbitron",
    img: "/img/minimalist-focus-workspace.png",
    currentTrack: "Soft Jazz",
    artist: "Evening Trio",
    albumArt: "/img/album2.png",
    playlist: ["Soft Jazz", "Night Breeze", "City Lights"],
  },
  {
    id: 3,
    name: "Creative Energy",
    tags: ["Inspire", "Bright"],
    background: "/img/new-custom-space.png",
    clockStyle: "retro",
    clockFont: "VT323",
    img: "/img/new-custom-space.png",
    currentTrack: "Indie Wave",
    artist: "Dreaming Kids",
    albumArt: "/img/album3.png",
    playlist: ["Indie Wave", "Morning Drive", "Pop Dreams"],
  },
  {
    id: 4,
    name: "Creative Energy",
    tags: ["Inspire", "Bright"],
    background: "/img/peaceful-meditation-space.png",
    clockStyle: "retro",
    clockFont: "VT323",
    img: "/img/peaceful-meditation-space.png",
    currentTrack: "Indie Wave",
    artist: "Dreaming Kids",
    albumArt: "/img/album3.png",
    playlist: ["Indie Wave", "Morning Drive", "Pop Dreams"],
  },

]
